...
class StandardComplianceAgent(ComplianceAgent):
    ...
    async def analyze(self, content: str, rules: List[Rule], execution_id: str = None, db: Any = None) -> ComplianceAnalysisResult:
        ...
        result = await ollama_service.generate_structured_response(
            prompt=prompt,
            output_model=ComplianceAnalysisResult,
            system_prompt=f"You are a specialist {self.category} compliance agent. Return ONLY valid JSON.",
            execution_id=execution_id,
            db=db,
            tool_name=f"{self.category}_agent"
        )
